...
function VerifyEmail() {
  ...
  const email = state?.email || 'Hello@tyler.com';

  const [verificationCode, setVerificationCode] = useState('');
  const [timeLeft, setTimeLeft] = useState(183); // 3:03 en segundos
      ...
      <div className="relative h-screen w-auto aspect-9/16 max-w-2xl overflow-hidden shadow-2xl bg-white">
        ...
        <div className="h-full bg-white flex flex-col px-6 py-8 overflow-y-auto">
          ...
          <div className="text-center mb-8">
            {/* Ilustración */}
            <img
              src="/images/Intruciones/verificar.png"
              alt="Verificación de email"
              className="w-75 h-auto mx-auto mb-6"
            />

            <h1 className="text-gray-800 text-3xl font-bold mb-2">
              Confirma tu Correo<br />Electrónico
            </h1>

            <p className="text-gray-600 text-base leading-relaxed">
              Te hemos enviado un código de verificación de 5 dígitos a{' '}
              <span className="font-semibold">{email}</span>. Introduce el código para completar tu registro.
            </p>
          </div>
